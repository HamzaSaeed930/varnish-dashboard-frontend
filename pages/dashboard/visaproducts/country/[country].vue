<template>
  <DashboardLayout>
    <div class="space-y-8 sm:space-y-4">
      ...
      >
        <div class="flex flex-col items-center gap-3">
          <p class="text-sm text-gray-600 dark:text-gray-400">No visa products found for {{ countryName }}</p>
          <button
            @click="navigateToAddVisaProduct"
            class="px-4 py-2 text-sm font-medium rounded-[6px] text-white bg-black dark:bg-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors"
          >
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
              ...
              >
                ...
                <td
                  class="px-4 py-3 text-sm text-[#475467] dark:text-white"
                >
                  {{ visaProduct.totalAmount }}
                </td>
                <td class="px-4 py-3">
                  <div class="flex items-center space-x-2">
                    <button
                      @click="viewVisaProduct(visaProduct.id)"
                    ...
                    <button
                      @click="editVisaProduct(visaProduct.id)"
                      class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                      title="Edit"
                    >
                      <svg
      ...
      >
        ...
        >
          {{ selectedCount }} of {{ visaProductsByCountry.length }} row(s) selected.
        </div>
...
const countryName = computed(() => {
  ...
  return formattedCountry ? formattedCountry.charAt(0).toUpperCase() + formattedCountry.slice(1) : '';
});

// Set page title
useHead({
  title: `${countryName.value} Visa Products - Varnish Dashboard`,
});
...
const loadVisaProducts = async () => {
  try {
    ...
    const response = await getVisaProductsByCountry(countryName.value);
    
    if (response.success && response.data) {
      // Map API data to include selected property
      visaProductsByCountry.value = response.data.map((product) => ({
        ...product,
        selected: false,
      }));
    } else {
      visaProductsByCountry.value = [];
...
const filteredVisaProducts = computed(() => {
  if (!searchQuery.value) return visaProductsByCountry.value;

  return visaProductsByCountry.value.filter((visaProduct) =>
    visaProduct.productName?.toLowerCase().includes(searchQuery.value.toLowerCase())
  );
});

const selectedCount = computed(() => {
  return visaProductsByCountry.value.filter((visaProduct) => visaProduct.selected).length;
});
...
const viewVisaProduct = (id: number | string) => {
  router.push(`/dashboard/visaproducts/add?id=${id}&mode=view`);
};
...
watch(selectAll, (newValue) => {
  visaProductsByCountry.value.forEach((visaProduct) => {
